...
A fixed conformer seed does not freeze generated chemistry or parameters. The
historical score table is retained as a diagnostic, while numerical scoring
oracles live in the analytical term and native/reference comparison tests.
This workflow checks that persistence preserves every score lane and gradient.
"""
...
def test_noncanonical_parameter_replay_and_minimization(
    backbone_class, torch_device, tmp_path, record_property
):
    ...
    for database in (prepared, restored):
        ...
        # Compare parameter replay in double precision on CUDA, where the
        # float32 score reductions are not bitwise deterministic.
        dtype = torch.float64 if torch_device.type == "cuda" else torch.float32
        coords = pose.coords.detach().to(dtype=dtype).clone().requires_grad_()
    ...
    historical = yaml.safe_load(data_path("noncanonical_scores.yaml").read_text())[
        backbone_class
    ]
    record_property(
        "historical_score_delta",
        json.dumps({name: value - historical[name] for name, value in by_name.items()}),
    )
