...
def _apply_mol2_metadata(mol, text):
    ...
    _resolve_mol2_delocalized_oxygens(mol, declared_charges, delocalized_bonds)
    return declared_charges


def _resolve_mol2_delocalized_oxygens(mol, charges, delocalized_bonds):
    """Use declared charges to localize acyclic carbonate/carboxylate/phosphate O.co2."""
    for center in mol.GetAtoms():
        if (center.GetAtomicNum(), center.GetDegree()) not in ((6, 3), (15, 4)):
            continue
        oxygens = [
            atom
...
def nonstandard_residue_info_from_mol2_block(
    ...
    charges = [
        float(a.GetProp("_TriposPartialCharge"))
        for a in mol.GetAtoms()
        if a.HasProp("_TriposPartialCharge")
    ]
    total_charge = sum(charges)
    mismatch = (
        _charge_model_is_authoritative(charge_model)
        and len(charges) == mol.GetNumAtoms()
        and np.isfinite(charges).all()
        and abs(total_charge - round(total_charge)) < 0.01
        and round(total_charge) != Chem.GetFormalCharge(mol)
    )
...
def _nonstandard_residue_info_from_mol2_mol(
    ...
    *,
    ...
    use_input_charges = (
        ...
        and abs(sum(partial_charges.values()) - Chem.GetFormalCharge(mol)) < 0.01
    )
...
def with_resolved_coordinates(atom_array, res_name: str, use_ccd: bool):
    ...
    Their coordinates define stereochemistry even for a wholly unresolved
    component; they do not place that component in the pose. Partially observed
    components require three anchors for alignment. Supplied coordinates and
    the caller's NaNs remain unchanged.
    """
