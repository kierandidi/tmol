...
def _metal_cross_residue_bonds(structure):
    ...
    for atom1, atom2, _bond_type in array.bonds.as_array():
        ...
        if res1 == res2:
            continue
        is_metal1 = keys[res1][3] in METAL_RESIDUE_NAMES
        is_metal2 = keys[res2][3] in METAL_RESIDUE_NAMES
...
def augment_database_for_metal_bonds(structure, param_db):
            ...
            if res_name in METAL_RESIDUE_NAMES:
                added_icoors = tuple(
                    _metal_connection_icoor(
                        ...
                        name,
                        atom,
                        float(np.linalg.norm(remote_coords[(input_key, name)] - local[atom])),
                    )
                    for atom, name in signature
                )
            else:
                ...
                for atom, name in signature:
                    ...
                    try:
                        icoor = attr.evolve(
                            _connection_icoor(raw, atom, local, remote), name=name
                        )
                    except ValueError:
                        icoor = _fallback_connection_icoor(
                            raw, name, atom, distance
                        )
                    added_icoors.append(icoor)
                added_icoors = tuple(added_icoors)
            clone_atoms, clone_bonds, clone_properties = _virtualize_leaving_hydrogens(
    ...
    if missing:
        raise ValueError(f"no residue type supports metal attachment pattern(s): {missing}")
...
def apply_metal_bonds_from_biotite(pose_stack, structure, variant_names):
    ...
    for pose_index in range(len(pose_stack)):
        for input_key, connections in endpoint_connections.items():
            ...
            if tuple(a.name for a in clone.atoms) != tuple(a.name for a in original.atoms):
            ...
            if torch.any(missing):
                virtual_inds = [clone.atom_to_idx[name] for name in clone.properties.virtual]
                anchor_candidates = [
                    i
