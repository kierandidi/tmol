...
_MEDIUM_PACKING_POSE_CHUNK = 10
_LARGE_PACKING_POSE_CHUNK = 10
...
def _max_poses_per_packing_chunk(pose_stack: PoseStack) -> int:
    ...
    and also limits transient score/index storage. The size tiers and memory
    ...
    if max_n_blocks <= 256:
        chunk_size = _SMALL_PACKING_POSE_CHUNK
    elif max_n_blocks <= 1024:
        chunk_size = _MEDIUM_PACKING_POSE_CHUNK
    else:
        chunk_size = _LARGE_PACKING_POSE_CHUNK
...
def _slice_pose_stack_for_packing(
    ...
    """
    if pose_stack.constraint_set is not None or pose_stack.split_block_mapping:
