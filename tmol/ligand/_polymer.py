...
def _specialize_from_parent(
    ...
    specialized_atoms = tuple(
        attr.evolve(atom, atom_type="Vrt") if atom.name in leaving_hydrogens else atom
    ...
    properties = attr.evolve(
        ...
        virtual=tuple(
            dict.fromkeys((*generated.properties.virtual, *leaving_hydrogens))
        ),
    )
    specialized = attr.evolve(
        ...
        icoors=(*generated.icoors, *parent_icoors),
    ...
    for atom_name in leaving_hydrogens:
        charges[atom_name] = 0.0
    return replace(
        preparation,
