...
def _has_atom_references(residue_types, patches=()):
    return any(
        atom.genbonded_type is not None or atom.cartbonded_reference is not None
        for atoms in (
...
def _write_rosetta_params_file(
    ...
    """
    if _has_atom_references((restype,)):
        raise ValueError(
            "Rosetta .params cannot preserve genbonded_type or cartbonded_reference; use .tmol"
        )
    lines: list[str] = []
...
def _write_tmol_params_file(
    ...
    has_generic_references = _has_atom_references(residue_types, patches or ())
